...
func wrapStep(name string, fn func(context.Context) error) {
...
func main() {
	...
	switch action {
	case "backend":
		wrapStep("backend", buildBackend)
	case "frontend":
		wrapStep("frontend", buildFrontend)
	case "image":
		wrapStep("backend", buildBackend)
		wrapStep("frontend", buildFrontend)
		wrapStep("image", buildAndPushImage)
	case "deploy":
		// wrapStep("backend", buildBackend)
		// wrapStep("frontend", buildFrontend)
		// wrapStep("image", buildAndPushImage)
		wrapStep("deploy", deploy)
	default:
...
func buildBackend(ctx context.Context) error {
	...
	for _, target := range []string{"api", "commander", "agent"} {
		golang = golang.Exec(dagger.ContainerExecOpts{
			Args: []string{"go", "build", "-o", path + target, "cmd/" + target + "/main.go"},
		})
	}
